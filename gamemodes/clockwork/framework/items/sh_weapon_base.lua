...
function ITEM:OnHandleUnequip(Callback)
	if (self.OnDrop) then
		local menu = DermaMenu();
			...
			menu:AddOption(L("Разоружить"), function()
				Callback();
			end);
			menu:AddOption(L("Выбросить"), function()
				Callback("drop");
			end);
		menu:Open();
	else
...
if (CLIENT) then
	function ITEM:GetClientSideInfo()
		...
		if (clipOne > 0) then
			clientSideInfo = Clockwork.kernel:AddMarkupLine(clientSideInfo, L("В обойме: ", clipOne));
		end;
		
		if (clipTwo > 0) then
			clientSideInfo = Clockwork.kernel:AddMarkupLine(clientSideInfo, L("Подствольник: ", clipTwo));
		end;
