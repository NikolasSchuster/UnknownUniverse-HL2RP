	...
	if (!factionTable.GetName) then
		if (!factionTable.useFullName) then
			if (data.forename and data.surname) then
				...
				if (!string.find(data.forename, "[aeiouауоиэы]") or !string.find(data.surname, "[aeiouауоиэы]")) then
					return self:SetCreateFault(player, {"FaultNameHaveVowel"});
				end;
				
				if (string.utf8len(data.forename) < 4 or string.utf8len(data.surname) < 4) then
					return self:SetCreateFault(player, {"FaultNameMinLength"});
				end;
				
				if (string.utf8len(data.forename) > 32 or string.utf8len(data.surname) > 32) then
