...
function Clockwork:ChatBoxAdjustInfo(info)
	...
	if (info.class == "ic") then
		cwKernel:PrintLog(LOGTYPE_GENERIC, {"#1 сказал \"#2\"", info.speaker:Name(), info.text});
	elseif (info.class == "looc") then
		cwKernel:PrintLog(LOGTYPE_GENERIC, {"[LOOC] #1: #2", info.speaker:Name(), info.text});
	end;
end;
