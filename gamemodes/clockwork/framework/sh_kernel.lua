...
if (SERVER) then
	...
	function Clockwork.kernel:HandleItemSpawnIconClick(itemTable, spawnIcon, Callback)
		...
		if (itemTable.OnUse and !itemTable.noInventoryUse) then
			itemFunctions[#itemFunctions + 1] = itemTable("useText", "Использовать");
		end;
		
		if (itemTable.OnDrop) then
			itemFunctions[#itemFunctions + 1] = itemTable("dropText", "Выбросить");
		end;
		
		if (itemTable.OnDestroy) then
			itemFunctions[#itemFunctions + 1] = itemTable("destroyText", "Уничтожить");
		end;
...
function Clockwork.kernel:ModifyPhysDesc(description)
	if (stringLen(description) <= 99) then
		if (!stringFind(stringSub(description, -1), "%p")) then
			return description..".";
		else
			return description;
		end;
	else
		return stringSub(description, 1, 96).."...";
	end;
end;
