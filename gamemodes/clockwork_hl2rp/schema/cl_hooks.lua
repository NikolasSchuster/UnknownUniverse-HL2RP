...
function Schema:GetCinematicIntroInfo()
	return {
		credits = "Разработчики сервера USPEX.",
...
function Schema:GetPlayerScoreboardClass(player)
	...
	elseif (faction == FACTION_RAIDER) then
		return "Рейдеры";
	elseif (faction == FACTION_CWU or faction == FACTION_CWUMED or faction == FACTION_CWUBOSS) then
		return "Гражданский Союз Рабочих";
	elseif (faction == FACTION_REBEL or faction == FACTION_REBELMED or faction == FACTION_REBELBOSS or faction == FACTION_VORTIGAUNT) then
		return "Сопротивление";
	elseif (faction == FACTION_SYNDICATE) then
		return "Синдикат";
	elseif (faction == FACTION_ENSLAVEDVORTIGAUNT) then
		return "Вортигонты";
	...
	elseif (faction == FACTION_SCANNER) then
		return "Синтеты";
...
function Schema:GetPlayerInfoText(playerInfoText)
	...
	if (citizenID) then
		if (Clockwork.Client:GetFaction() == FACTION_CITIZEN) then
			playerInfoText:Add("CITIZEN_ID", L("Персональный ID: #", citizenID));
		end;
	end;
end;
...
function Schema:GetScreenTextInfo()
	...
	if (Clockwork.Client:GetSharedVar("PermaKilled")) then
		return {
			...
			title = L("ВАШ ПЕРСОНАЖ ПОГИБ"),
			text = L("Перейдите в меню персонажей, чтобы выбрать другого.")
		};
	elseif (Clockwork.Client:GetSharedVar("BeingTied")) then
		return {
			alpha = 255 - blackFadeAlpha,
			title = L("ВАС СВЯЗЫВАЮТ")
		};
